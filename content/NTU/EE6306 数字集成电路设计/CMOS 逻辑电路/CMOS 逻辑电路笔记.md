...
#### 2.1.3.2. 互补 CMOS 门的两个主要问题

首先，实现一个具有 N 个输入（扇入）的门需要的晶体管数量是 $2N$，显著增加了面积、寄生电容等。其次，互补 CMOS 门的传播延时随扇入数迅速增加（想想串联的 NMOS！）。
...
在有比逻辑中，整个 PUN 被一个无条件的负载器件所替代，用于始终上拉输出以得到高电平输出。例如使用电阻或者栅极接地的 PMOS 负载，第二种也叫做伪 NMOS 门。
